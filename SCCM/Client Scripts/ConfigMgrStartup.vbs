...
Const MSG_VERIFY_WRITEABLE_LOGSHARE_PASSED = "The Log share is writeable"
'= Functions =
...
'		End If
	...
	'test that the CONFIGMGRLOGPATH is writeable and get the final log path
	DestLogPath = getFinalLogPath(configOptions)
	
	WriteLogMsg "Script execution complete.", 1, 1, 0
	WriteFinalLogMsg configOptions
	
	MoveFinalLogToLogShare g_logPathandName, DestLogPath, configOptions
...
Function Return_ClientVersion(configOptions)
	Dim wmi, ccmWMI, errorCode, errorMsg
	Dim clientProperties, clientProp
	Dim sActualClientVersion, expectedVersion
	Dim configMgrLogPath
  ...
  errorCode = Err.Number
	...
	'return
	If bContinue = True Then
		Return_ClientVersion = sActualClientVersion
	Else
		Return_ClientVersion = False
	End If
End Function
...
Function UninstallClient(ByRef options, ByRef parameters)
	...
	commandLine = "C:\windows\ccmsetup\ccmsetup.exe"
	...
	If returnCode = true Then
	  WriteLogMsg "Uninstall failed: " & returnCode, 3, 1, 0
	  unInstallClient = False
	Else
	  WriteLogMsg "Uninstall succeeded.", 1, 1, 0
...
Function InstallClient(ByRef options, ByRef parameters)
	...
	Dim installPatchProperty
	Dim errorCode, errorMsg, msg
	Dim bAction, bInstalled, clientVer
		
	installPatchProperty = ""
	WriteLogMsg MSG_INSTALLCLIENT_START, 1, 1, 0
	
	'check version again
	bInstalled = Check_IsClientInstalled(options)
	If bInstalled = vbTrue Then
		clientVer = Return_ClientVersion(options)
		If Left(clientVer,1) = "4" Then
			bAction = UninstallClient(options, parameters)
		End If
	End If
...
Sub MoveFinalLogToLogShare(ByVal g_logPathandName,ByVal DestLogPath, ByRef options)
	'Variables
	Dim strDate
	Dim paddedDate
	Dim strPadDate
	...
	TrimTrailingSlash DestLogPath
	'Attach the Winfs\logs UNC path to the new log file name.
	g_finalLog  = DestLogPath & "\" & strLogName
...
Function IsPathWriteable(ByVal ConfigOptions)
	...
	'Attach the Winfs\logs UNC path to the new log file name.
	g_finalLog  = ConfigOptions.Item(CONFIGMGRLOGPATH) & "\" & strLogName
	
	
	'Open existing log file and prep it for writing
	On Error resume next
		Err.Clear
		set remoteLog = g_fso.OpenTextFile(g_finalLog, 2, true, true) 
		if Err.Number <> 0 Then
			msg = "Error: the target log location " & ConfigOptions.Item(CONFIGMGRLOGPATH) & " failed, using C drive for final log storage"
			IsPathWriteable = false
		else
			msg = MSG_VERIFY_WRITEABLE_LOGSHARE_PASSED
			IsPathWriteable = true
		end if
	On Error goto 0

	WriteLogMsg msg, 1, 1, 0
	if (not (IsEmpty(remoteLog)))  then
		remoteLog.close
	end if
	
End Function

Function getFinalLogPath(ByVal configOptions)
	dim msg
	
	if IsPathWriteable(configOptions) = false then
		getFinalLogPath = "C:\"
		msg = "Final log will be moved to the C drive"
	else
		getFinalLogPath = configOptions.Item(CONFIGMGRLOGPATH)
	end if
	WriteLogMsg msg, 1, 1, 0
End Function
