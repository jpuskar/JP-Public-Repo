...
$logfile = "C:\Windows\Logs\lmstatParser.log"

Function Throw-Message($msg){
	$msg > $logfile
}

If((Test-Path $optionsFilePath) -eq $false) {
	$msg = "Error`tCould not find lm daemon options file """ + $optionsFilePath + """."
	Throw-Message $msg
}

If((Test-Path $lmstatOutputFile) -eq $false) {
	$msg = "Error`tCould not find lmstat output file """ + $lmstatOutputFile + """."
	Throw-Message $msg
}
...
$oGroupReservations = Generate-GroupReservationMap
#$oGroupReservations
...
$LMFeatures | % {
	...
	$groupList | % {
		...
		$InUse = 0
		$currentUsers = $null
		$LMFeatureReservationInstances | ?{$_.FeatureName -eq $featureName -and $_.GroupName -eq $curGroupName.ToLower()} | % {
			$InUse++
			If($currentUsers -notcontains $_.Username) {
				$currentUsers += $_.Username; $currentUsers += " "
			}
		}
		...
		$ReportItem | Add-Member -Type NoteProperty -Name "Current Users" -Value $currentUsers
...
If((Test-Path $scriptCSVOutputFilename) -eq $false) {
	$msg = "Error`tCould not write CSV output file """ + $scriptCSVOutputFilename + """."
	Throw-Message $msg
}
