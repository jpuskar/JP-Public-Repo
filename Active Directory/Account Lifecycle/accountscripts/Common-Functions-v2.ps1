...
Function Run-RemoteCommand($server,$command,$bSkipReturn)
	{
		If($bSkipReturn -ne $true){$bSkipReturn = $false}
		$fail = $false
		$hshRetCodes = @{}
		$hshRetCodes.Add(0,"Successful Start")
		$hshRetCodes.Add(2,"Access Denied")
		$hshRetCodes.Add(3,"Insufficient Privilege")
		$hshRetCodes.Add(8,"Unknown Failure")
		$hshRetCodes.Add(9,"Path Not Found")
		$hshRetCodes.Add(21,"Invalid Parameter")
		...
		[int32]$retCode = $objProcess.ReturnValue
		$global:Run1ExitStatus = $null
		$null = Get-EventSubscriber | where {$_.sourceIdentifier -eq "RemoteCommandRun1"} | unregister-event | out-host
		
		If($retCode -eq 0)
			{$null = Register-WmiEvent -SourceIdentifier "RemoteCommandRun1" -ComputerName $server -Query ("Select * from Win32_ProcessStopTrace Where ProcessID=" + $ID) -Action { $global:Run1ExitStatus = $($event.SourceEventArgs.NewEvent.ExitStatus) }}
		Else
			{
				$retMessage = $null
				$possibleRetCodes = $hshRetCodes.Keys
				If($possibleRetCodes -contains $retCode)
					{$retMessage = $hshRetCodes.Get_Item($retCode)}
				Else
		...
		If($fail -eq $False)
			{
				...
				#decide if we're on the remote system or not, then wait until the event ends.
				If($computer -eq $server)
					{
		...
		If($fail -eq $false -and $bSkipReturn -eq $false)
			{
				$i = 0
				While($global:Run1ExitStatus -eq $null -or $global:Run1ExitStatus -eq "")
					{
				...
				If($exitcode -ne 1){$exitcode = $global:Run1ExitStatus}
			}
		Else
			{$exitcode = $retcode}
		
		#write-host -f yellow "ding"
		#$exitcode | gm | out-host
		#$exitcode = 15
